 ...
 * went rather than pushing its stale copy back. Last write wins per record,
 * which is the right rule for a store one person edits from two rooms.
 *
 ...
 * lists them so the client can find files no record describes.
 */
...
function fresh() {
  return { v: 3, rev: 0, nextNo: 1, records: new Map(), tombstones: new Map() }
}
...
function changesSince(s, since) {
  ...
  return { rev: s.rev, nextNo: s.nextNo, full, records, removed }
}
  ...
  const handler = async (req, res, next) => {
    ...
    try {
      ...
      if ((p === '/api/archive/upsert' || p === '/api/archive/restore') && req.method === 'POST') {
        ...
        const s = await load()
        const assigned = upsert(s, b.records)
        if (assigned.length) { save(); broadcast(s.rev) }
        return send(res, 200, { rev: s.rev, nextNo: s.nextNo, assigned })
      }
      ...
      if (p === '/api/outputs' && req.method === 'GET') {
        ...
        return send(res, 200, { root: OUTPUTS, files: since ? files.filter(f => f.mtime > since) : files })
      }
