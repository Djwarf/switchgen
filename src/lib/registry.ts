// Originally generated by the model-params-and-wiring swarm, now hand-maintained:
// there is no scripts/gen-registry, so edit this file directly and re-run
// `npx tsx scripts/validate-workflows.ts` (needs ComfyUI up) to check every graph
// against the live /object_info.
//
// When changing sampler settings, keep the distilled and full configs separate.
// Wan 2.2 14B full (20 steps, no Lightning LoRA) wants shift 8.0; the 4-step
// Lightning presets want shift 5.0. Mixing them is a silent quality bug.
...
export const FAMILY_DEFS: FamilyDef[] = [
  ...
  },
  {
    ...
    "perModel": {
      ...
      "lightning-4step": {
        "_desc": "NOT the graph as emitted. Requires adding both Lightning LoRAs at strength 1.0. Distilled: shift 5 and cfg 1 belong to this preset only.",
        "steps": 4,
        "split": 2,
        "cfg": 1.0,
        "shift": 5.0,
      ...
      "base-no-lora": {
        "_default": true,
        "_desc": "This IS the graph as emitted: 20 steps, split 10, no Lightning LoRA. shift 8.0. PRIMARY SOURCE: comfy/supported_models.py:1311-1319 WAN21_T2V.sampling_settings shift 8.0. Wan 2.2 14B loads the Wan 2.1 VAE (16 latent channels) so it resolves to WAN21_T2V/WAN21_I2V, not WAN22_T2V (which needs out_dim 48, i.e. the 5B); both inherit 8.0 regardless. 8.0 is therefore what ComfyUI applies if ModelSamplingSD3 is deleted entirely. TIEBREAKER: video_wan2_2_14B_flf2v.json loads the SAME wan2.2_i2v 14B high/low checkpoints and deliberately carries two shift values - 8 on its 20-step path, 5 on its 4-step path - so ComfyUI template authors treat 8 as the non-distilled value for these exact weights. Also shift 8 at 20 steps in _fun_control, _fun_camera, _s2v and all 5B templates. KNOWN COUNTER-EVIDENCE, recorded deliberately: video_wan2_2_14B_t2v.json and _i2v.json DEFAULT to the non-distilled 20-step branch (PrimitiveBoolean \"Enable Lightning LoRA\" = false driving ComfySwitchNodes; nodes_logic.py:115 selected = on_true if switch else on_false) and still ship shift 5.0 as a single unswitched widget shared by both branches. And _fun_inpaint ships shift 8 on an ACTIVE 4-step distilled branch. So shift 5 is NOT a reliable distillation marker, and the majority/model-class argument is what carries this, not a clean rule. Do not flip back to 5.0 without new evidence.",
        "steps": 20,
        "split": 10,
        "cfg": 3.5,
        "cfg_alt_swarmui": 5.0,
    ...
    "notes": "Q4 pair with a quantised umt5 encoder. This family loads NO LoRA - both experts run bare, so they are consistent with each other. RAM is still the binding constraint: two 14B experts at ~9.3 GB each plus the encoder puts a 49-frame 832x480 render near 20 GB RSS, and earlyoom SIGTERMs at mem <= 8% free (~28.1 GB used of 30.5). Measured 2026-09-22: a second render in the same long-lived ComfyUI process was OOM-killed at 19989 MiB RSS because memory accumulates across runs; restart the service between heavy renders.",
    "graph": {
      "1": {
        "class_type": "UnetLoaderGGUF",
        "inputs": {
  ...
  },
  {
    ...
    "notes": "Q4 pair with a quantised umt5 encoder. MEASURED: peak 28.1 GB of 30.5 GB with a LoRA on ONE half; a LoRA on both halves was OOM-killed. The measurement stands, but the reason previously recorded here ('LoRA patching dequantises GGUF weights') is WRONG and was verified false in source: custom_nodes/ComfyUI-GGUF/ops.py:166-191 get_weight() is called from cast_bias_weight() inside forward_*, so a GGUF tensor stays quantised in RAM and is dequantised transiently per-tensor per-forward, with the LoRA held as tensor.patches metadata. Resident cost of the second LoRA is the file itself (613,516,752 B = 0.61 GB), not a model dequantisation. The real constraint is that 28.1 GB already sits on earlyoom's trigger (SIGTERM at mem <= 8% free, i.e. ~28.1 GB used of 30.5), so any addition tips it. Both experts now carry their matched LoRA half (HIGH on node 17, LOW on node 5, both 0.85). MEASURED 2026-09-22, three runs, all killed at the same point - 11-12s after 'Requested to load WanVAE', i.e. the VAE cannot load while both 9.3 GB experts are resident. Tiled decode does NOT help (it shrinks per-tile activations, not the VAE load). Results: 81 frames + 2 LoRAs = OOM; 49 frames + 2 LoRAs = SUCCESS; 49 frames + 1 LoRA = OOM. The lighter config failing at the same length proves this is NON-DETERMINISTIC, not a length or LoRA-count threshold: earlyoom fires only when mem <= 8% AND swap <= 10%, so survival depends on zram swap headroom at decode time. Do not read the one success as proof 49 is safe. CONCLUSION: this family is not reliable on 30 GB. For image-to-video prefer the 5B, which Video.tsx deriveImageToVideo() already supports (Wan22ImageToVideoLatent is in START_FRAME_NODES): measured 1280x704, 49 frames, 186s, peak 8.6 GB - higher resolution, 4.6x faster, and no OOM. Length is 49 here only to improve the odds, not because 49 was proven safe.",
    "graph": {
      "1": {
        "class_type": "UnetLoaderGGUF",
        "inputs": {
      ...
      "17": {
        ...
        "inputs": {
          ...
          "lora_name": "Wan22_I2V_NSFW_General_HIGH.safetensors",
          "strength_model": 0.85
        }
      },
      "3": {
